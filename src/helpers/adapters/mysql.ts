...
export async function getSpaceFollowers() {
  const query = `
    SELECT following as space, COUNT(id) as count, count(IF(created > (UNIX_TIMESTAMP() - 86400), 1, NULL)) as count_1d FROM follows GROUP BY following
  `;
